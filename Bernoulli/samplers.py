...
class ModifiedDiGsSampler(nn.Module):
    def __init__(self, dim, n_steps=10, approx=False, multi_hop=False, fixed_proposal=False, temp=2., step_size=0.2,
        ...
        self.score_based_sampler = ModifiedDiGsScoreSamp(self.dim, self.L, fixed_proposal=False, approx=True,
                                                         multi_hop=False,
                                                         temp=2., alpha=self.step_size, eta=self.eta, mh=True)
    ...
    def step(self, x, x_a, model):
        ...
        for i in range(self.n_steps):
            ...
            for k in range(self.K):
                ...
                e2=torch.norm(x_a_cur - x_delta,dim=1).mean().item()
                e1 = torch.norm(x_a_cur - x_cur, dim=1).mean().item()
                print(i,k,e1, e2,cur_hops)
                """
...
class ModifiedDiGsSampler1(nn.Module):
    def __init__(self, dim, n_steps=10, approx=False, multi_hop=False, fixed_proposal=False, temp=2., step_size=0.2,
                 mh=True, K=1, step_size_a=0.5, eta1=10 ** 4, eta2=1,alpha=0.1, score_sweeps=50, n=1):
        super().__init__()
        self.dim = dim
        self.n_steps = n_steps
        self._ar = 0.
        ...
        self.score_based_sampler = ModifiedDiGsScoreSamp(self.dim, self.L, fixed_proposal=False, approx=True,
                                                         multi_hop=False,
                                                         temp=2., alpha=self.step_size, eta=self.eta1, mh=True)
    ...
    def step(self, x, x_a, model,step):
        ...
        for i in range(self.n_steps):
            ...
            for k in range(self.K):
                uniform = torch.rand_like(x_cur)
                logistic_noise = torch.log(uniform / (1 - uniform))
                x_a_cur = x_cur + self.eta1 * logistic_noise

                x_a_cur1=x_a_cur +self.alpha*torch.tanh((x_a_cur - x_cur) / 2 * self.eta1) / (self.eta1 )
                ...
                # Compute stabilized diff values
                diff_0 = (x_a_cur1 - 0) / (2 * self.eta2)  # Energy difference for 0
                diff_1 = (x_a_cur1 - 1) / (2 * self.eta2)  # Energy difference for 1
            ...
            if self.mh:
                q_forward = (x_a_cur1 - x_cur) / (2 * self.eta2)
                lp_forward = -2 * torch.log(torch.cosh(q_forward) + EPS).sum(dim=1)

                q_reverse = (x_a_cur1 - x_delta) / (2 * self.eta2)
                lp_reverse = -2 * torch.log(torch.cosh(q_reverse) + EPS).sum(dim=1)

                m_term = (model(x_delta).squeeze() - 2 * torch.log(
                    torch.cosh((x_a_cur - x_delta) / (2 * self.eta1))).sum(dim=1)) - (
                                     model(x_cur).squeeze() - 2 * torch.log(
                                 torch.cosh((x_a_cur - x_cur) / (2 * self.eta1))).sum(
                                 dim=1))
...
class ModifiedDiGsScoreSamp2(nn.Module):
    def __init__(self, dim, n_steps=10, approx=False, multi_hop=False, fixed_proposal=False, temp=2., alpha=0.2,
                 mh=True, eta=10 ** 4):
        super().__init__()
        self.dim = dim
    ...
    def step(self, x, x_a, model):
        ...
        for i in range(self.n_steps):
            ...
            grad_x_cur_modified = grad_x_cur + (1/self.temp)*((2*(x_a_cur-x_cur))/(self.eta**2 +((x_a_cur-x_cur)**2)) )* -(2 * x_cur - 1)
            ...
            if self.mh:
                ...
                grad_x_delta_modified = grad_x_delta + (1/self.temp)*((2*(x_a_cur-x_delta))/(self.eta**2 +((x_a_cur-x_delta)**2)) )* -(2 * x_delta - 1)

                flip_prob = torch.exp(grad_x_delta_modified - term2) / (torch.exp(grad_x_delta_modified - term2) + 1)
                probs = flip_prob * ind + (1 - flip_prob) * (1. - ind)
                lp_reverse = torch.sum(torch.log(probs + EPS), dim=-1)

                m_term = (model(x_delta).squeeze() - 1 * torch.log(1 +
                                                                   torch.sum(
                                                                       torch.pow((x_a_cur - x_delta) / (1 * self.eta),
                                                                                 2), dim=1))) - (
                                 model(x_cur).squeeze() - 1 * torch.log(1 +
                                                                        torch.sum(torch.pow(
                                                                            (x_a_cur - x_cur) / (1 * self.eta), 2),
                                                                                  dim=1)))
        ...
        return (x_cur, x_a_cur)  # Update x, not x_a
class ModifiedDiGsSampler2(nn.Module):
    def __init__(self, dim, n_steps=10, approx=False, multi_hop=False, fixed_proposal=False, temp=2., step_size=0.2,
                 mh=True, K=1, step_size_a=0.5, eta=10 ** 4, score_sweeps=50, n=1):
        super().__init__()
        self.dim = dim
        ...
        self.K = K
        self.mh = mh
        self.score_based_sampler = ModifiedDiGsScoreSamp2(self.dim, self.L, fixed_proposal=False, approx=True,
                                                         multi_hop=False,
                                                         temp=2., alpha=self.step_size, eta=self.eta, mh=True)
    ...
    def step(self, x, x_a, model,step):
        ...
        for i in range(self.n_steps):
            ...
            for k in range(self.K):
                ...
                # Generate standard Cauchy noise
                cauchy_noise = torch.tan(torch.pi * (uniform - 0.5))  # Standard Cauchy(0, 1)

                # Scale and shift the noise to mimic Cauchy(x_cur, eta)
                x_a_cur = x_cur + self.eta * cauchy_noise



                energy_0 = -1 * torch.log(1+
                    (torch.pow((x_a_cur - 0) / (1 * self.eta),2)))  # Energy for 0
                energy_1 = -1 * torch.log(1+
                    (torch.pow((x_a_cur - 1) / (1 * self.eta),2)))
            ...
            if self.mh:
                q_forward = torch.sum(torch.pow((x_a_cur - x_cur) / (1 * self.eta),2),dim=1) +1
                lp_forward = -1 * torch.log((q_forward) )

                q_reverse = torch.sum(torch.pow((x_a_cur - x_delta) / (1 * self.eta),2),dim=1) +1
                lp_reverse = -1 * torch.log((q_reverse) )

                m_term = (model(x_delta).squeeze() - 1 * torch.log(1+
                    torch.sum(torch.pow((x_a_cur - x_delta) / (1 * self.eta),2),dim=1))) - (
                                     model(x_cur).squeeze() - 1 * torch.log(1+
                    torch.sum(torch.pow((x_a_cur - x_cur) / (1 * self.eta),2),dim=1)))
