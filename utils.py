...
import os
from vamp_utils import load_caltech101silhouettes
...
def approx_difference_function(x, model):
    ...
    #print(x,model(x),gx)
    #print(model.gradient(x))
    wx = gx * -(2. * x - 1)
    return wx.detach()

def auxillary_gradient_function(eta,x,x_a):
    ...
    if args.data in potential_datasets:
        ...
        if args.data in ["mnist", "fashion", "emnist", "kmnist", "omniglot"]:
            ...
            if args.data == "caltech":
                (train_loader,_,test_loader,args,) = load_caltech101silhouettes(args)
