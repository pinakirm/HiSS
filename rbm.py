...
def next_nearest_neighbor_adjacency(L, flat_regions=None, sharp_regions=None):
    N = L * L
    W = torch.zeros((N, N))
    ...
    for x in range(L):
        for y in range(L):
            ...
            for nx, ny in neighbors:
                neighbor_idx = index(nx, ny)

                # Default weight
                weight = 0.5

                # Adjust weights for flat regions
                if flat_regions and (x, y) in flat_regions:
                    weight = 0.2

                # Adjust weights for sharp regions
                if sharp_regions and (x, y) in sharp_regions:
                    weight = 1.0

                W[idx, neighbor_idx] = weight

    return W


def generalized_ising_parameter_generation(d):
    # Create a tensor of shape (d, d) filled with random values between -1 and 1
    tensor = torch.rand(d, d) * 2 - 1

    # Ensure the diagonal entries are 0
...
class BernoulliRBM(nn.Module):
    def __init__(self, n_visible, n_hidden, data_mean=None):
        ...
        """
        self.W = nn.Parameter(torch.randn(n_hidden, n_visible) * 25.0)  # Large values
        sparsity_level =0.9 # 70% of weights are zero
        mask = (torch.rand_like(self.W.data) > sparsity_level).float()
        self.W.data *= mask
        print(f"W shape: {self.W.shape}")

        #self.b_h = nn.Parameter(torch.zeros(n_hidden,))
        self.b_h = nn.Parameter(torch.randn(n_hidden) * 5)
        self.b_v = nn.Parameter(torch.randn(n_visible) * -5)  # Strongly negative biases
        """
...
class LatticeIsingModel(nn.Module):
    def __init__(self, dim, init_sigma=.15, init_bias=0., learn_G=False, learn_sigma=False, learn_bias=False,
        ...
        self.G=torch.zeros(dim**2, dim**2,device=device)


        rows = torch.arange(self.G.size(0))
        cols = torch.arange(self.G.size(1) - 1, -1, -1)
        self.G[rows, cols] = 10

        torch.diagonal(self.G, 0).fill_(10)
        ...
        #self.sigma,self.bias=generalized_ising_parameter_generation(dim**2)
